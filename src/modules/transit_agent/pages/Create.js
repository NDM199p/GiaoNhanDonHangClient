...
function CreateTransitAgentPage() {
  ...
  let formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(TransitAgentAction.add({ transitAgentObject: values }));
    },
  });

  return (
    <>
      {transitAgentSelector.isLoading && (
        <div
        ...
        >
          <Spinner animation="btransitAgent" variant="light" />
        </div>
      )}

      {transitAgentSelector.transitAgentError && (
        <Alert variant="danger">{transitAgentSelector.error.message}</Alert>
      )}
...
export default CreateTransitAgentPage;
